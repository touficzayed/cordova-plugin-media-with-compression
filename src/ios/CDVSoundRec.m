...
- (CDVAudioFileRec*)audioFileForResource:(NSString*)resourcePath withId:(NSString*)mediaId doValidation:(BOOL)bValidate forRecording:(BOOL)bRecord
{
    ...
    if (bError) {
        jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%@);", @"cordova.require('cordova-media-with-compression.MediaRec').onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode:errcode message:errMsg]];
        ...
        if ((audioFile != nil) && (audioFile.resourceURL != nil)) {
            ...
            if (!bError) {
                ...
                if (!bError) {
                    ...
                    jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%.3f);\n%@(\"%@\",%d,%d);", @"cordova.require('cordova-media-with-compression.MediaRec').onStatus", mediaId, MEDIA_DURATION, position, @"cordova.require('cordova-media-with-compression.MediaRec').onStatus", mediaId, MEDIA_STATE, MEDIA_RUNNING];
                    [self.commandDelegate evalJs:jsString];
                }
            }
            if (bError) {
                ...
                // jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%d);", @"cordova.require('cordova-media-with-compression.MediaRec').onStatus", mediaId, MEDIA_ERROR,  MEDIA_ERR_NONE_SUPPORTED];
                jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%@);", @"cordova.require('cordova-media-with-compression.MediaRec').onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode:MEDIA_ERR_NONE_SUPPORTED message:nil]];
                [self.commandDelegate evalJs:jsString];
            }
        }
...
- (void)stopPlayingAudio:(CDVInvokedUrlCommand*)command
{
    ...
    if ((audioFile != nil) && (audioFile.player != nil)) {
        ...
        [audioFile.player stop];
        audioFile.player.currentTime = 0;
        jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%d);", @"cordova.require('cordova-media-with-compression.MediaRec').onStatus", mediaId, MEDIA_STATE, MEDIA_STOPPED];
...
- (void)pausePlayingAudio:(CDVInvokedUrlCommand*)command
{
    ...
    if ((audioFile != nil) && (audioFile.player != nil)) {
        ...
        jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%d);", @"cordova.require('cordova-media-with-compression.MediaRec').onStatus", mediaId, MEDIA_STATE, MEDIA_PAUSED];
    }
    // ignore if no media playing
...
- (void)seekToAudio:(CDVInvokedUrlCommand*)command
{
    ...
    if ((audioFile != nil) && (audioFile.player != nil)) {
        ...
        if (posInSeconds >= audioFile.player.duration) {
            ...
            [audioFile.player stop];
            audioFile.player.currentTime = 0;
            jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%.3f);\n%@(\"%@\",%d,%d);", @"cordova.require('cordova-media-with-compression.MediaRec').onStatus", mediaId, MEDIA_POSITION, 0.0, @"cordova.require('cordova-media-with-compression.MediaRec').onStatus", mediaId, MEDIA_STATE, MEDIA_STOPPED];
            // NSLog(@"seekToEndJsString=%@",jsString);
        } else {
            audioFile.player.currentTime = posInSeconds;
            jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%f);", @"cordova.require('cordova-media-with-compression.MediaRec').onStatus", mediaId, MEDIA_POSITION, posInSeconds];
            ...
            void (^startRecording)(void) = ^{
                ...
                if (error == nil) {
                    ...
                    if (recordingSuccess) {
                        ...
                        jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%d);", @"cordova.require('cordova-media-with-compression.MediaRec').onStatus", mediaId, MEDIA_STATE, MEDIA_RUNNING];
                        [weakSelf.commandDelegate evalJs:jsString];
                    }
                }
